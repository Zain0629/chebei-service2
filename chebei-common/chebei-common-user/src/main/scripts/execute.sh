...
APP_MAIN_CLASS=com.chebei.user.ChebeiUserRun

#拼凑完整的classpath参数，包括指定lib目录下所有的jar
CLASSPATH=${CLASSPATH}:${APP_HOME}:${APP_HOME}/classes:${APP_HOME}/conf:${APP_HOME}/setting
...
#java虚拟机启动参数
JAVA_OPTS=""
JAVA_OPTS=${JAVA_OPTS}"-server -Xmx2048M -Xms512M -XX:MaxNewSize=1024M "
...
start() {
	...
	if [ ${psId} -ne 0 ]; then
		...
		JAVA_CMD="nohup $JAVA_HOME/bin/java $JAVA_OPTS -classpath $CLASSPATH $APP_MAIN_CLASS >/dev/null 2>&1 &"
